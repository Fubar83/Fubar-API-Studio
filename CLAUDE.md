...
```
Fubar.Controls  (shared, app-agnostic Avalonia component library; + Fubar.Controls.Gallery sandbox)
      ▲ consumed by
...
- `Fubar.Controls` depends on **none** of the app layers (Avalonia + AvaloniaEdit only).
...
- **Small, composable components**: build larger views/controls from small single-purpose ones. Generic,
  reusable UI pieces go in `Fubar.Controls` (with a `Fubar.Controls.Gallery` example); anything bound to
  a domain concept (e.g. the `VariableTooltip`/`VariableIntellisense` behaviors) stays app-side.
...
| Reusable controls + theme/design system | `src/Fubar.Controls` (sandbox: `src/Fubar.Controls.Gallery`) |
...
```bash
dotnet build Fubar.slnx                       # whole solution (must be warning-clean)
dotnet test  Fubar.slnx                        # all tests (see below)
...
`Fubar.Studio.Infrastructure.Tests` (v2), `Fubar.Controls.Tests` (v3, headless Avalonia), and
...
- **Don't break the `Fubar.Controls` boundary** — the arch test enforces it, and the whole point of the
  library is that it drops into any Avalonia app.
